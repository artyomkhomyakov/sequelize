      ...
      if (options.individualHooks) {
        ...
        if (options.updateOnDuplicate) {
          ...
          if (options.upsertIndex !== undefined) {
            options.upsertKeys = _.chain(model.primaryKeys).values().map('field').value();              const upsertIndex = model._indexes.find(index => index.name === options.upsertIndex);
            if (Object.keys(model.uniqueKeys).length > 0) {             if (upsertIndex === undefined) {
              options.upsertKeys = _.chain(model.uniqueKeys).values().filter(c => c.fields.length >= 1).map(c => c.fields).reduce(c => c[0]).value();               throw new Error(`upsertIndex '${options.upsertIndex}' not defined.`);
            }
